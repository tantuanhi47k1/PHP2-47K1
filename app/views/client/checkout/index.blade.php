...
@section('content')
    <style>
        .payment-option input:checked+label { color: #0d6efd; }
        .payment-option:hover { background-color: #f8f9fa !important; border-color: #0d6efd !important; }
        .payment-option input:checked { border-color: #0d6efd; background-color: #0d6efd; }
        .border-bottom-dashed { border-bottom: 1px dashed #dee2e6 !important; }
        .btn-place-order:hover { transform: translateY(-2px); box-shadow: 0 10px 20px rgba(13, 110, 253, 0.3) !important; }
    </style>
    ...
    <div class="container py-5">
        ...
        <form action="/checkout/process" method="POST" id="checkoutForm">
            ...
            <input type="hidden" name="coupon_code" id="input-coupon-code" value="">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" name="phone" class="form-control bg-light"
                                        placeholder="09xxxx..." value="{{ $old['phone'] ?? ($user['phone'] ?? '') }}" required>
                        ...
                        <div class="card-body p-4">
                            ...
                            <div class="bg-light p-3 rounded-3 mb-4">
                                ...
                                <div class="form-check p-3 bg-white border rounded mb-2 payment-option">
                                    ...
                                    <label class="form-check-label w-100 d-flex justify-content-between align-items-center" for="paymentCod" style="cursor: pointer;">
                                ...
                                <div class="form-check p-3 bg-white border rounded mb-2 payment-option">
                                    ...
                                    <label class="form-check-label w-100 d-flex justify-content-between align-items-center" for="paymentVnpay" style="cursor: pointer;">
                                ...
                                <div class="form-check p-3 bg-white border rounded mb-2 payment-option">
                                    ...
                                    <label class="form-check-label w-100 d-flex justify-content-between align-items-center" for="paymentMomo" style="cursor: pointer;">
                                        <span class="fw-semibold" style="color: #a50064;">Ví MoMo</span>
                                        <img src="https://avatars.githubusercontent.com/u/36770798?s=200&v=4" height="30" class="rounded" alt="MoMo">
                                    </label>
                                </div>
                                <div class="form-check p-3 bg-white border rounded payment-option">
                                    ...
                                    <label class="form-check-label w-100 d-flex justify-content-between align-items-center" for="paymentBank" style="cursor: pointer;">
                                        <span><i class="bi bi-bank me-2 text-secondary"></i>Chuyển khoản ngân hàng</span>
                                    </label>
                                </div>
                            </div>

                            <button type="submit" class="btn btn-primary w-100 py-3 fw-bold text-uppercase shadow-lg rounded-pill btn-place-order">
    ...
    <script>
        ...
        function renderCheckoutCart() {
            ...
            const discountAmount = parseFloat(localStorage.getItem('discount_amount')) || 0;
            
            $('#cart_data_input').val(JSON.stringify(cart));
            $('#input-coupon-code').val(couponCode);
            $('#input-discount-amount').val(discountAmount);

            if (cart.length === 0) {
                $('#cart-list').html('<li class="list-group-item text-center py-5"><i class="bi bi-cart-x text-danger fs-1"></i><p class="mt-3 text-muted">Giỏ hàng trống!</p><a href="/shop" class="btn btn-sm btn-outline-primary mt-2">Quay lại mua sắm</a></li>');
            ...
            cart.forEach(item => {
                ...
                if (item.image && item.image !== '') {
                    img = item.image.startsWith('/') || item.image.startsWith('http') ? item.image : '/' + item.image;
                }
            ...
            html += `<li class="list-group-item d-flex justify-content-between pt-3 pb-1 px-0 border-0 text-muted"><span>Tạm tính</span><span>${new Intl.NumberFormat('vi-VN').format(subTotalAmount)}đ</span></li>`;

            if (couponCode && discountAmount > 0) {
                html += `<li class="list-group-item d-flex justify-content-between py-1 px-0 border-0 text-success"><span>Mã giảm giá (<b>${couponCode}</b>)</span><span>-${new Intl.NumberFormat('vi-VN').format(discountAmount)}đ</span></li>`;
            }

            let finalTotal = subTotalAmount - discountAmount;
            if(finalTotal < 0) finalTotal = 0;

            html += `<li class="list-group-item d-flex justify-content-between py-3 px-0 border-0"><span class="fw-bold fs-5">Tổng cộng</span><strong class="text-danger fs-4">${new Intl.NumberFormat('vi-VN').format(finalTotal)}đ</strong></li>`;
        ...
        function renderData(array, select) {
            ...
            array.forEach(element => { row += `<option data-name="${element.name}" value="${element.code}">${element.name}</option>` });
        ...
        $("#province").change(() => {
            ...
            if (provinceCode) {
                ...
                axios.get(host + "p/" + provinceCode + "?depth=2").then((response) => { renderData(response.data.districts, "district"); });
        ...
        $("#district").change(() => {
            ...
            if (districtCode) {
                ...
                axios.get(host + "d/" + districtCode + "?depth=2").then((response) => { renderData(response.data.wards, "ward"); });
            } else {
                $("#ward").prop("disabled", true);
            }
            updateFullAddress();
        ...
        $("#ward, #address_detail").on('change input', () => { updateFullAddress(); });
        ...
        function updateFullAddress() {
            ...
            let parts = [detail, ward, district, province];
            $("#full_address").val(parts.filter(part => part && part.trim() !== "").join(", "));
        }
    </script>
@endsection